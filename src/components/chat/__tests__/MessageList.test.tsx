...
test("MessageList renders an empty list when no messages", () => {
  ...
  // Empty state UI lives in ChatInterface, not MessageList.
  // MessageList itself renders an empty messages wrapper.
  const messageWrappers = container.querySelectorAll(".rounded-xl");
  expect(messageWrappers).toHaveLength(0);
});
